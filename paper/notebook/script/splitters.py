...
def cv_random_split(dataset, data_name,fold_idx = 0,
                   frac_train=0.8, frac_valid=0.2, seed=0,
                 smiles_list=None):
    """
    ...
    for idx in skf.split(labels):
        idx_list.append(idx)
    #train_idx, val_idx = idx_list[fold_idx]
    if os.path.isfile('data/'+data_name+'_split_folds.json'):
        print('split already exist!')
    else:
        with open('data/'+data_name+"_split_folds.json","w") as f:
            print('new split')
            idx_dict = {}
            for i,fold in enumerate(idx_list):
                print(i)
                print(fold)
                print(fold[0],type(fold[0]))
                temp = {}
                temp['train']=list(map(int,list(fold[0])))
                temp['test']=list(map(int,list(fold[1])))
                idx_dict[i] = temp
            print(idx_dict)
