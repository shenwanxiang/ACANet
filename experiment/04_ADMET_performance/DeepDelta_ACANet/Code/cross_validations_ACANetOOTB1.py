...
from models import ACANetOOTB, ACANetOPT 
...
def cross_validation(x, y, prop, model, k=10, seed=1): # provide option to cross validate with x and y instead of file
    ...
    for train, test in kf.split(x):
        ...
        if seed == 1: # Saving individual folds for mathematical invariants analysis
            results = [preds]
            #pd.DataFrame(results).to_csv("../Results/ACANetOOTB/{}_{}_Individual_Fold_{}.csv".format(prop, model, cnt), index=False)
...
for prop in properties:
    dirpath = os.path.join('./tmp/acanetootb', prop)
    models = [ACANetOOTB(dirpath=dirpath, gpuid=0)]
    for model in models:
        ...
        for i in range(5): # Allow for 5x10-fold cross validation
            ...
            pd.DataFrame(results).to_csv("../Results/ACANetOOTB/{}_{}_{}.csv".format(prop, str(model), i), index=False) # Save results
            ...
            df = pd.read_csv("../Results/ACANetOOTB/{}_{}_{}.csv".format(prop, model, i)).T
        ...
        delta.to_csv("../Results/ACANetOOTB/{}_{}_delta_scoring.csv".format(prop, model)) # Save data